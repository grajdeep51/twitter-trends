...
def tweet_location(tweet):
    ...
    tweet_lat = tweet.get('latitude')
    tweet_lon = tweet.get('longitude')
    return make_position(tweet_lat, tweet_lon)
...
def extract_words(text):
    ...
    return word_list # Replace this line
...
def analyze_tweet_sentiment(tweet):
    ...
    for word in words:
        ...
        if has_sentiment(senti):
            total_senti.append(sentiment_value(senti))
    if len(total_senti) == 0:
        return make_sentiment(None)
    average = sum(total_senti) / len(total_senti)
...
def average_sentiments(tweets_by_state):
    ...
    for s in tweets_by_state:
        ...
        if average is not None:
            averaged_state_sentiments[s] = average
